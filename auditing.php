	...
	if(is_array($row))
    {
	        ...
	        if(is_array($row2))
            {
			      if($row2['standardlevel']<$row['jibie'])
			      {
					  ...
					  {
					  //如果是新会员
					  	$sql3="select * from shengji where userid='".$row['userid']."' and jibie=1 and jieuserid<>'".$rowuser['id']."' and passed=1 limit 1";
...
//qqqqqq
//找出滑落点位
//$sql12="select * from users where shunxu>0 and nextusernum<3 order by shunxu asc ";//全网排
...
//qqqqqq
					  ...
					  {
					  //如果是老会员
					    $sql3="select * from shengji where userid='".$row['userid']."' and jibie='".$row['jibie']."' and id<>'".$row['id']."' and passed=1 limit 1";
    				  	$query3=$db->query($sql3);
					  	$row3=$db->fetch_array($query3);
					  	if(is_array($row3))
    				  	{
...
?>
                ...
                <td><?php echo $rowu['nickname']?><br><span class="text-yellow"><?php echo $rowu['wechat']?></span><br><span class="text-blue"><?php echo $rowu['loginname']?></span></td>
                <td><?php echo userjibiename($row['jibie'])?>(<? if($row['types']==1){echo "<font color=#b96d0a>升级</font>";}else{echo "<font color=blue>核实</font>";}?>)</td>
                <td><?php echo $row['addtime']?></td>
                <td>
					                    	<span class="text-green"><? if($row['passed']==1){echo "已通过";}else{echo "<a href=?act=qr&id=".$row['id']." target='formprocess' onClick='javascript:return confirm(&#39;确认通过？&#39;);' target='formprocess'><font color=red>确认</font></a>";}?></span>
                                    </td>
            </tr>
<?
}
