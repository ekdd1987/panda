...
{
	$delsql="delete from adhgfqws65ljdlgr where ID ='".$_GET['ArticleID']."' ";
	$db->query($delsql);
...
if($_POST['ArticleID']&&$_GET['Action']=="Del")
{
	...
	$delsql="delete from adhgfqws65ljdlgr where ID in($ids)";
	$db->query($delsql);
	echo "<script>alert('删除成功!');window.location.href='?types=".$_GET['types']."&page=".$_GET['page']."';</script>";
	exit;
}
...
if($Title)$sqladd.=" and username like '%".trim($Title)."%' ";
$sqladd.=' ORDER BY ID DESC';

$perNumber=20; //每页显示的记录数
$page=$_GET['page']; //获得当前的页面值
$count=$db->query("select count(*) from adhgfqws65ljdlgr where 1=1 {$sqladd}"); //获得记录总数
...
$result=$db->query("select * from adhgfqws65ljdlgr where 1=1 {$sqladd} limit $startCount,$perNumber"); //根据前面的计算出开始的记录和记录数
?>
...
　</td>
        ...
        <table class="border" border="0" cellspacing="2" width="98%" cellpadding="0" style="word-break:break-all">
          <tr  class="title"> 
            <td width="63" height="25" align="center"><strong>选中</strong></td>
            <td width="262" align="center"  ><strong>用户名</strong></td>
...
?>
          <tr class="tdbg"> 
            <td width="63" height="22" align="center"> <input name='ArticleID[]' type='checkbox' onclick="unselectall()" id="ArticleID[]" value='<? echo $row['ID']?>'></td>
            <td align="center">&nbsp;<a href="newsModify.php?ArticleID=<? echo $row['ID']?>"><? echo $row['username']?></a></td>
            <td width="133" align="center"> 
              <a href="newsModify.php?ArticleID=<? echo $row['ID']?>&types=<? echo $_GET['types']?>&page=<? echo $page?>">修改</a> 
              <a href="?ArticleID=<? echo $row['ID']?>&Action=Del&types=<? echo $_GET['types']?>&page=<? echo $page?>" onClick="javascript:return confirm(&#39;确定删除吗？&#39;);">删除</a>            </td>
          </tr>
<?
}
?>
...
<input class="p_input" type="button" name="button" id="button" value="跳转" onclick="window.location='?page='+custompage.value;" /></td>
      ...
      <br> 
      <table width="98%" border="0" cellpadding="0" cellspacing="0" class="border">
        <tr class="tdbg">
