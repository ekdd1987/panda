...
class users{
	...
	//获取会员团队达标等级
	public function getstandardlevel($level)
	{	 
		$bizset = new bizset();
		$bizsetinfo = $bizset->get_bizset_bykey("STANDARD_LEVEL".$level);
		$level = $bizsetinfo["memo"];
		return $level;
	}
	//添加用户
	public function add_user($db,$loginname,$pwd,$pid,$sel,$qq,$pwd2,$pwd3,$truename,$identityid,$bank,$bankno,$bankname,$bankaddress,$tel,$qq,$rid,$area,$grade)
	{
		...
		identityid,bank,bankno,bankname,bankaddress,tel,qq,rid,area,addtime,productcount,standardlevel) values
				   ('".$pid."','".$sel."','".$loginname."','".$pwd."','".$pwd2."','".$pwd3."','".$truename."','".$identityid."','".$bank."','".$bankno."','".$bankname."','".$bankaddress."','".$tel."','".$qq."','".$rid."','".$area."',now(),".$grade.",1)");
		//user_sub_count添加记录
		$users = new users();
	...
	public function user_upgrade_stand()
	{
		...
		$count=$db->num_rows($query);
	  	for($i=0;$i<$count;$i++)
	  	{
			$row = $db->fetch_array($query);
			$uid = $row["id"];
			$productcount = $row["standardlevel"];//团队达标级别
	...
	//三个市场累计业绩比较 取最小值
	public function getMinNum($s1,$s2,$s3)
	{
		if($s1<$s2) 
		{
			$min=$s1;
	...
	//根据用户pid 递归取所有用户编号 不包括自己
	public function getrecursiveallusersid_notme($uid)
	{			
	    $ls_return = "";
...
/*		$bizsetinfo = $bizset->get_bizset_bykey("STANDARD");//达标业绩个数
		...
		//按级别分红
		self::award_standard_settle($db,1);
		self::award_standard_settle($db,2);
		self::award_standard_settle($db,3);
		self::award_standard_settle($db,4);
		self::award_standard_settle($db,5);
		self::award_standard_settle($db,6);
		self::award_standard_settle($db,7);
		self::award_standard_settle($db,8);
		self::award_standard_settle($db,9);
		self::award_standard_settle($db,10);
		self::award_standard_settle($db,11);
	...
	public function divi_settle($db,$level)
	{
		...
		$dividendamount = $allamount*$recpercent;
 		//echo "select abs(sum(amount)) as amount  from income where types='BUYSTOCK' and  addtime >='$day_begin' and addtime <='".$day_end." 23:59:59"."'";exit;
	...
	//达标团队加权平均分红奖 周分红
	public function award_standard_settle($db,$level)
	{ 
		$users = new users();
		$bizset = new bizset();
		$award = new award();
		...
		//根据等级获取达标分红比例
		$bizsetinfo = $bizset->get_bizset_bykey("STANDARD_PERCENT".$level);
		$standard_percent = $bizsetinfo["bizvalue"];

		//本周日期
		//echo '本周第一天（星期日为一周开始）：'.date('Y-m-d', time()-86400*date('w'))';
		//echo '本周第一天（星期一为一周开始）：'.date('Y-m-d', time()-86400*date('w')+(date('w')>0?86400:-6*86400));
		$day_begin = date('Y-m-d', time()-86400*date('w')+(date('w')>0?86400:-6*86400));
		$day_end = date('Y-m-d', time()-86400*date('w'));
		...
		$dividendamount = $allamount*$standard_percent;	
		//echo $allamount."-".$dividendamount ;exit;
		
		//B=同级别所有会员本期团队新增业绩的总和  
		//$query=$db->query("select abs(sum(amount)) as amount  from income where types='BUYSTOCK'  and (settlementno is null or settlementno='') and userid in(select id from users where standardlevel = $level)");
		$query=$db->query("select abs(sum(amount)) as amount  from income where types='BUYSTOCK'  and addtime >='$day_begin' and addtime <='".$day_end." 23:59:59"."'  and userid in(select id from users where standardlevel = $level)");
		$result = $db->fetch_array($query);
		...
		if($this_amount>0)
		{
			//根据standardlevel等级 遍历users 
			$query=$db->query("select*from users where states = 1 and standardlevel = $level");
			$count=$db->num_rows($query);
			for($i=0;$i<$count;$i++)
			{
				$row = $db->fetch_array($query);
				...
				//A=该级别会员本期团队新增业绩
				$query_u=$db->query("select abs(sum(amount)) as amount  from income where types='BUYSTOCK'  and addtime >='$day_begin' and addtime <='".$day_end." 23:59:59"."'  and  userid = $uid ");
				$result_u = $db->fetch_array($query_u);
				$useramount = $result_u["amount"];
